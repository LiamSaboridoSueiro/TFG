...
SUJETOS_FILE = Path("data/checks/sujetos_aptos.txt")   # generado por check_integridad_dataset.py
...
def leer_sujetos_aptos() -> list[str]:
    """Lee la lista de sujetos aptos generada por check_integridad_dataset.py."""
    if not SUJETOS_FILE.exists():
        raise FileNotFoundError(
            f"No se encontró {SUJETOS_FILE}. "
            "Ejecuta primero check_integridad_dataset.py"
        )
    sujetos = []
...
def buscar_fif(sujeto_id: str, condicion: str) -> Path | None:
...
def calcular_psd_bandas(epochs: mne.Epochs) -> np.ndarray:
    """
...
def normalizar_features_sujeto(features_por_cond: dict, mode: str = ZSCORE_MODE) -> tuple[dict, np.ndarray, np.ndarray]:
    """
...
def obtener_pares_alpha_asymmetry(ch_names: list[str]) -> list[tuple[str, str, int, int]]:
...
def construir_bloques_features(deltas: dict, ch_names: list[str]) -> list[tuple[str, dict, list[str]]]:
    """
    ...
    if INCLUDE_ALPHA_ASYMMETRY:
        pares = obtener_pares_alpha_asymmetry(ch_names)  # lista de (left, right, left_idx, right_idx)
        if pares:
            alpha_asym = {}
            for cond, delta in deltas.items():
...
def extraer_psd_todos(sujetos: list[str]) -> dict:
    """
    ...
    for sujeto in sujetos:
        ...
        for cond in CONDITIONS:
            contador += 1
            fif_path = buscar_fif(sujeto, cond)
            ...
            psd_lin = calcular_psd_bandas(ep_eeg)   # (n_ep, n_ch, n_bands) en escala lineal V²/Hz
...
# ---------------------------------------------------------------------- Log10
def aplicar_log10(psd_lineal: np.ndarray) -> np.ndarray:
    """
...
def calcular_delta(datos_sujeto: dict) -> dict:
    """
    ...
    psd_neutro_log = aplicar_log10(datos_sujeto["NEUTRO"]["psd_lineal"])
    ...
    for cond in CONDITIONS:
        if cond not in datos_sujeto:
            continue
        psd_log = aplicar_log10(datos_sujeto[cond]["psd_lineal"])
...
def ensamblar_dataset(sujetos: list[str], datos: dict) -> tuple:
    """
    ...
    for sujeto in sujetos:
        ...
        try:
            deltas, _ = calcular_delta(datos[sujeto])
        ...
        bloques  = construir_bloques_features(deltas, ch_names)
        ...
        for block_name, block_cond, block_names in bloques:
            block_z, mu, sigma = normalizar_features_sujeto(block_cond, mode=ZSCORE_MODE)
...
def diagnostico_features(X, y, meta):
...
if __name__ == "__main__":
    ...
    X, y, meta, zscore_params, feature_names, feature_blocks = ensamblar_dataset(sujetos, datos)
