...
# ---------------------------------------------------------------------- CARGAR EL FICHERO EDF

if len(sys.argv) < 2:
    raise RuntimeError("Uso: python edf_to_epochs.py <ruta_edf>")

edf_path = Path(sys.argv[1]).resolve()
...
output_dir = Path("../../data/processed/epochs") / emotion
...
stats_dir = Path("../data/processed/preprocessing_stats")
