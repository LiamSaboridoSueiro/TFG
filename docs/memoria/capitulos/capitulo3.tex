...
Un modelo de caja negra es aquel cuyo funcionamiento interno no resulta fácil de seguir por una persona. Aunque se conocen las características que recibe como entrada y la predicción que genera como salida, no siempre es sencillo entender cómo transforma esas variables en una decisión concreta. Esto suele ocurrir en modelos con una estructura compleja, como redes neuronales profundas\footnote{\url{https://en.wikipedia.org/wiki/Neural_network_(machine_learning)}} o métodos de conjunto como Random Forest\footnote{\url{https://en.wikipedia.org/wiki/Random_forest}}, donde la relación entre entrada y salida queda distribuida entre numerosos parámetros o transformaciones internas \cite{molnar2022interpretable}. Un ejemplo conocido de sistema basado en modelos de este tipo es ChatGPT\footnote{\url{https://openai.com/chatgpt/}}: el usuario puede observar el texto que introduce y la respuesta que recibe, pero el proceso interno que genera esa respuesta no se presenta como una secuencia de reglas fácilmente interpretable.
...
En este trabajo se consideran dos clasificadores interpretables: regresión logística\footnote{\url{https://en.wikipedia.org/wiki/Logistic_regression}} y árbol de decisión\footnote{\url{https://en.wikipedia.org/wiki/Decision_tree_learning}}. Ambos parten de la misma idea general: reciben una muestra representada por sus características y asignan una clase de salida. Sin embargo, cada uno aprende esa relación de una forma distinta.
...
\textbf{Árbol de decisión.} Un árbol de decisión clasifica las muestras mediante una secuencia de reglas. Su estructura está formada por nodos, ramas y hojas: en cada nodo se evalúa una condición sobre una característica, cada rama representa el resultado de esa condición y cada hoja contiene la clase asignada. Por ejemplo, el modelo puede dividir primero las muestras según el valor de una característica concreta y, después, seguir refinando la decisión con nuevas condiciones.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.55\textwidth]{figs/Arbol_decision.png}
    \caption{Ejemplo de estructura de árbol de decisión. Fuente: \cite{adrigm2013adivinador}.}
...
La validación cruzada \textit{k-fold}\footnote{\url{https://en.wikipedia.org/wiki/Cross-validation_(statistics)}} extiende esta idea de separación entre entrenamiento y prueba. En lugar de realizar una única división, el conjunto de datos se divide en \(k\) particiones o \textit{folds}. En cada iteración, el modelo se entrena con \(k-1\) particiones y se evalúa con la partición restante. Al repetir el proceso hasta que todos los \textit{folds} han actuado como conjunto de prueba, se obtiene una estimación media del comportamiento del modelo y de su variabilidad entre particiones \cite{kohavi1995crossvalidation}.
