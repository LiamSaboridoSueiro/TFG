...
\]
\caption{Contribución \acs{SHAP} lineal utilizada para la regresión logística en cada fold.}
\label{eq:shap_lineal}
\end{myequation}

donde \(w_j^{(c)}\) es el coeficiente aprendido por la regresión logística para esa clase, \(x_j\) es el valor de la característica en la muestra explicada y \(\bar{x}_{j,\mathrm{train}}\) es la media de esa característica en las muestras de entrenamiento del fold. De esta forma, la explicación conserva una relación directa con los coeficientes del modelo y con los valores reales de la muestra analizada.

\subsection{Agregación de importancias}

Una vez calculadas las explicaciones de todos los folds, el script reúne los valores \ac{SHAP} de cada sujeto. La agregación se realiza por clase usando las épocas cuya etiqueta real pertenece a esa condición emocional. Es decir, para resumir \texttt{JOY} se toman las épocas reales de \texttt{JOY} y las contribuciones asociadas a la salida de esa clase; el mismo criterio se aplica a \texttt{NEUTRO} y \texttt{SAD}.
