    ...
    \begin{columns}[c,onlytextwidth]
        \begin{column}{0.36\textwidth}
            ...
            \begin{tikzpicture}[
                ...
                \node[bloque, fill=blue!14] (filtro) {Filtrado y preparación};
                \node[detalle, below=0.03cm of filtro] (filtrod)
                    {canales, montaje, ruido eléctrico y remuestreo};

                \node[bloque, fill=orange!16, below=0.52cm of filtrod] (artefactos)
                    {Limpieza de artefactos};
                \node[detalle, below=0.03cm of artefactos] (artefactosd)
                    {canales problemáticos, referencia, ICA e interpolación};

                \node[bloque, fill=green!16, below=0.52cm of artefactosd] (epocas)
                    {Segmentación en épocas};
                \node[detalle, below=0.03cm of epocas]
                    {ventanas con información emocional útil};

                \draw[flecha] (filtrod) -- (artefactos);
                \draw[flecha] (artefactosd) -- (epocas);
            \end{tikzpicture}
        \end{column}
...
\begin{frame}{Extracción de características}
    \begin{columns}[c,onlytextwidth]
        ...
        \begin{column}{0.59\textwidth}
            ...
            \begin{tabular}{llc}
                \toprule
                \textbf{Bloque} & \textbf{Ejemplo} & \textbf{N} \\
                \midrule
                Potencia Alpha/Beta & \texttt{FC4\_Beta} & 124 \\
                Asimetría Alpha & \texttt{AlphaAsym\_F4-F3} & 26 \\
                \midrule
            ...
            \vspace{0.22cm}
            {\scriptsize La referencia NEUTRO y el escalado se calculan después, dentro de cada \textit{fold}.}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{Análisis de separabilidad}
    \centering
    \resizebox{0.98\textwidth}{!}{
    \begin{tikzpicture}[
        carril/.style={
        ...
        \path[carril, fill=violet!6] (-6.9,0.55) rectangle (-1.05,1.85);
        \path[carril, fill=blue!5]   (-6.9,-1.35) rectangle (-1.05,-0.05);

        \node[etiqueta, fill=violet!17] (global) at (-5.65,1.20) {
            Modelo general
        };

        \node[pasoent, fill=white] (globalpca) at (-2.65,1.20) {
            PCA y LDA\\
            global
        };

        \node[etiqueta, fill=blue!12] (sujeto) at (-5.65,-0.70) {
            Modelo\\
            por sujeto
        };

        \node[pasoent, fill=white] (sujetopca) at (-2.65,-0.70) {
            PCA y LDA\\
            por sujeto
        };

        \node[pasoent, fill=white] (condicion) at (0.35,0.25) {
            proyección\\
            por condición
        };

        \node[metricas] (met) at (4.95,0.25) {
            Separabilidad\\[-0.6mm]
            {\scriptsize silueta y centroides}
        };

        \draw[flecha] (global) -- (globalpca);
        \draw[flecha] (sujeto) -- (sujetopca);
        \draw[flecha] (globalpca.east) -- (condicion.north west);
        \draw[flecha] (sujetopca.east) -- (condicion.south west);
        \draw[flecha] (condicion) -- (met);
    \end{tikzpicture}
    }
\end{frame}
